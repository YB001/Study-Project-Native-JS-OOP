...
/* harmony import */ var _modules_difference_difference__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/difference/difference */ "./src/js/modules/difference/difference.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_difference_difference__WEBPACK_IMPORTED_MODULE_3__["default"]('.officerold', '.officernew', '.officer__card-item').init();
  new _modules_forms_forms__WEBPACK_IMPORTED_MODULE_4__["default"]('.form').init();
    ...
    value: function initMask() {
      ...
      function createMask(event) {
        ...
        this.value = matrix.replace(/./g, function (a) {
          return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : 1 >= val.length ? '' : a;
        });
  ...
  _createClass(VideoPlayer, [{
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            ...
            if (document.querySelector('iframe#frame')) {
              //при повторном включении плеера не создаем новый, а отображаем старый
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //параметр true включает "глубокое копирование" для копирования всех параметров элемента

        if (state.data === 0) {
          //разблокировываем второе видео после полного просмотра первого
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      if (n > this.slides.length) {
        //после последнего слайда переходим на первый слайд
        this.slideIndex = 1;
      }

      if (n < 1) {
        //после первого слайда возвращаемся на последний
        this.slideIndex = this.slideIndex.length;
      }

      try {
        //прорабатываем всплывающий блок на 3й странице
        this.blockHanson.style.opacity = '0'; //изначально скрываем блок

        if (n === 3) {
          // когда попадаем на 3-ю страницу, отображаем блок, дополнительно добавив анимацию
          this.blockHanson.classList.add('animated');
          setTimeout(function () {
            _this.blockHanson.style.opacity = '1';

            _this.blockHanson.classList.add('slideInUp');
          }, 3000);
        } else {
          //в случае ухода со страницы убираем класс
          this.blockHanson.classList.remove('slideInUp');
        }
      } catch (e) {}

      this.slides.forEach(function (slide) {
        //скрываем все слайды со страницы чтобы потом вывести один с нужным нам индексом
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (item) {
        //назначаем обработчик события для кнопки переключения слайдов
        item.addEventListener('click', function () {
          _this2.plusSlides(1);
  ...
  _createClass(MiniSlider, [{
    ...
    value: function nextSlide() {
      if (this.slides[1].tagName === "BUTTON" && this.slides[1].tagName === "BUTTON") {
        ...
        this.container.appendChild(this.slides[0]); //помещаем первый слайд в конец списка
...
var Slider = function Slider() {
  ...
  //устанавливаем null для начальных параметров, чтоб не возникало ошибок при отсутствии параметров
